...
/**
 * Copyright - Lloyd Sparkes 2012 LICENSE: Public Domain - do as you wish, just retain this message.
 * I just ask that if you find bugs or improve the code, you raise a push request or an issue, so i
 * can update the code for everyone DISCLAIMER: I am not responsible for your usage of this code, or
 * for any bugs or issues with this code or any resulting side effects
 *
 * <p>This class simply takes a POJO and creates the associated Proto Buffer Specification File.
 *
 * <p>Supports: Nested POJO's Enums Arrays/Collections/Lists/Sets (BUT only if they have a type
 * specifier!! (so List<Byte> is supported, List is not supported) Maps/KeyValuePairs (BUT they need
 * a type specifier!! (so Map<String,Integer> is supported, Map is not supported) Primitives Boxed
 * Primitives
 *
 * <p>Does Not Support: Nested Collections e.g. Map<List<String>,String> Arrays with more than one
 * Dimension
 *
 * <p>Usage - CLI: java -jar JavaToProto.jar JavaToProto <class name> [<output file name>]
 *
 * <p>If output file name is not specified the output will be to the console.
 *
 * <p>Ensure that the class name is in the class path somewhere.
 *
 * <p>Usage - Code:
 *
 * <p>JavaToProto jpt = new JavaToProto(class name); String protoFile = jpt.toString();
 *
...
public class JavaToProto {
    ...
    private Stack<Class> currentClassStack = new Stack<Class>();
    ...
    public JavaToProto(Class classToProcess) {
        ...
        currentClassStack.push(classToProcess);
    }
    ...
    public String getPath() {
        ...
        while (!currentClassStack.isEmpty()) {
            Class t = currentClassStack.pop();
            if (path.length() == 0) {
        ...
        while (!tStack.isEmpty()) {
            currentClassStack.push(tStack.pop());
        }
            ...
            for (int i = 0; i < paramType.length; i++) {
                ...
                // Check whether Method Parameter is an array.
                        ...
                        if (!typeMap.containsKey(innerType)) {
                        	buildNestedTypeforMethod(innerType);
                        }
    ...
    private void processFields() {
        ...
        for (Field f : fields) {
            ...
            // Check for List
                ...
                if (!typeMap.containsKey(innerType)) {
                	processRepeatedField(REPEATED, innerType.getSimpleName(), f.getName(), i);
                    classStack.push(innerType);
                }else {
                	processRepeatedField(REPEATED, typeMap.get(innerType), f.getName(), i);
                }
                continue;
	...
	private void buildNestedTypeforMethod(Class type) {
        currentClassStack.push(type);
        buildMessage();
        currentClassStack.pop();
    }
    ...
    private void processEnum(Class enumType) {
        ...
        currentClassStack.push(enumType);
    ...
    private void generateProtoFile() {
        ...
        buildMessage();
        currentClassStack.push(initialClass);
        buildClassMethod();
    }
